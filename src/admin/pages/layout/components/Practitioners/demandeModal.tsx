...
import { FaCertificate, FaEnvelope, FaIdCard, FaTimes, FaUser } from 'react-icons/fa';
...
const DemandeModal: React.FC<DemandeModalProps> = ({ practitioner, onClose }) => {
  ...
  const handleStatusChange = async (newStatus: string) => {
    try {
      await axios.put(`${API}/praticien/update/${practitioner.userId}`, {
        status: newStatus,
      });

      await axios.post(`${API}/praticien/envoyer-email`, {
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          {[
            ...
            { icon: FaIdCard, label: "CIN", value: practitioner.nationalIdNumber }
          ].map((item, index) => (
        ...
        <ModalButtons onStatusChange={handleStatusChange} />
      </motion.div>
    </motion.div>,
    document.body
  );
