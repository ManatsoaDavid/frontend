...
const SubscriptionTypeList: React.FC<SubscriptionTypeListProps> = ({ subscriptionTypes, onEdit, onDelete }) => {
  const getDescription = (type: string) => {
    switch (type) {
      case 'TRIAL':
        return 'Version d\'essai gratuite';
      case 'LITE':
        return 'Accès aux fonctionnalités de base';
      case 'PREMIUM':
        return 'Accès complet avec support prioritaire';
      case 'GOLD':
        return 'Accès VIP avec fonctionnalités exclusives';
      default:
              ...
              {subscriptionTypes.map((st: ISubscriptionType) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span className="text-sm text-gray-600">{getDescription(st.type)}</span>
                  </td>
