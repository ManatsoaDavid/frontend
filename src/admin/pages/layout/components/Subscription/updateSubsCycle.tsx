...
interface UpdateSubscriptionCycleModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (id: number, cycle: ESubscriptionCycle, duration: number) => void;
  subscriptionCycle: ISubscriptionCycle | null;
}

/**************************COMPOSANT PRINCIPAL*************************/
const UpdateSubscriptionCycleModal: React.FC<UpdateSubscriptionCycleModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
  subscriptionCycle
  ...
  useEffect(() => {
    if (subscriptionCycle) {
      setCycle(subscriptionCycle.cycle);
      setDuration(subscriptionCycle.duration.toString());
    }
  }, [subscriptionCycle]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!validateForm() || !subscriptionCycle?.subscriptionCycleId) return;

    try {
      setIsSubmitting(true);
      await onSubmit(subscriptionCycle.subscriptionCycleId, cycle, parseInt(duration));
      onClose();
    } catch (err) {
  ...
  if (!isOpen || !subscriptionCycle) return null;
      ...
      <AnimatePresence>
        ...
        >
          ...
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-2xl font-bold text-gray-800">Modifier le cycle</h3>
            <motion.button
...
export default UpdateSubscriptionCycleModal;
