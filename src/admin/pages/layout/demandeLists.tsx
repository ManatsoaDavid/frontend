...
const DemandeLists: React.FC = () => {
  ...
  const loading = useSelector((state: any) => state.praticiens.loading)
  const error = useSelector((state: any) => state.praticiens.error)
  const users = useSelector((state: any) => state.praticiens.users);
  ...
  const filteredUsers = Array.isArray(users) ? users.filter((practitioner: IPractitionerWithUser) =>
  ...
  /**************************FILTRAGE PAR STATUT*************************/
  const newRequests = filteredUsers.filter((practitioner: IPractitionerWithUser) =>
    practitioner.status === 'nouveau'

  );

  const pendingRequests = filteredUsers.filter((practitioner: IPractitionerWithUser) =>
    practitioner.status === 'attente'
  );

  const historyRequests = filteredUsers.filter((practitioner: IPractitionerWithUser) =>
            ...
            {activeTab === 'new' && newRequests.map((practitioner: IPractitionerWithUser) => (
              ...
              >
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <p><span className="font-medium">CIN:</span> {practitioner.nationalIdNumber || "N/A"}</p>
